...
For each evaluated algorithm, the rate of succesful predictions and the F-score are reported. In addition, the predictions are shown on a PCA projection
...
Table~\ref{table:color_validation} shows the mean error rates and F scores for each evaluated algorithm. The values are averaged across each
cross-validation partition on the training data.

\begin{table}[H]
\caption{Color prediction - validation set}
