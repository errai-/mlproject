...
The type and quality of wines is predicted using machine learning techniques. Predicting
the type is a pure classification problem, but the prediction of quality can be viewed either as
a classification or a regression problem. Various classification and regression algorithms
are used for analyzing the problems.
It turns out that predicting the type is a problem that can be solved
with very good accuracy. A hybrid model is constructed for making the final wine type predictions.
On the contrary, determining the quality of a wine is more challenging. Different
algorithms are evaluated and the final result is obtained using only one learning
algorithm. The obtained model performs somewhat well, but there is room for more optimization. 
...
a variety of different methods is used. This is important when dealing with a randomly chosen dataset, since according to
...
classification problems, whereas only the quality can be studied also using regression.
There is obviously some correlation between qualities that differ only by one point, which implies that
...
The used dataset is derived from the so called \emph{Vinho verde} data by Cortez et al. \cite{CorCer09}.
It consists of 6000 observations, each describing an individual wine. Values of 11 variables are provided in addition
...
are to be predicted. The former should be relatively easy, assuming that a sufficient set
...
Multiple machine learning methods are used for predicting the behavior of the wine data.
To some extent, the same classification algorithms are used for both analyses, with only minor differences.
Additionally, regression algorithms are used to predict the wine quality.
The basic properties of the used algorithms are described in this section. 
The theoretical background of most of the algorithms can be found
in a basic reference, such as Alpaydin\cite{Alpaydin}. 
...
learning algorithm. The most simple model always returns a constant value or
class. A slightly more complicated, but still simple model is provided by using only one of the eleven predictor variables. 
...
each class $k$ are obtained:
\begin{equation}
 p(x|\mu_k,\sigma_k) = \frac{1}{\sqrt{2\pi} \sigma_k} e^{-\frac{(x-\mu_k)^2}{2\sigma_k^2}}.
\end{equation}
This can be used directly as a likelihood function for the different classes. A Bayesian posterior
...
training set. The posterior can be transformed conveniently into a discriminant, by choosing a reference
class $j = k$:
...
one, extending the discriminant function to eleven variables. It determines the class of a sample by
...
Methods that rely on discrimination can directly model the class discriminants instead of
...
done numerically, for instance using the steepest descent algorithm. There are multiple 
ways to perform the optimization. One possibility
is logistic discrimination, where the class probabilities are defined by a connection to the class
probabilities.

\subsubsection{Quadratic discrimination}\label{method:qdiskr}

Quadratic discrimination is otherwise similar to linear discrimination, but it utilizes a quadratic discriminant function,
\begin{equation}
...
Support vector machines are a variant of linear discrimination. If the classes do not overlap, it finds
...
Random forests are an ensemble learning method that utilizes an ensemble of classification trees\cite{Forest}. Each tree 
in the ensemble does classification using only a chosen number of input variables. This could for
instance be the square root of the total amount of classification variables. For this kind of an ensemble
...
situation is more complicated. The final output is averaged over many trees, thus resembling a more continuous behavior.
...
\subsection{Principal component analysis}\label{method:pca}

Principal Component Analysis is a method that can be used with both classification and regression
...
hand, it provide an effective method for generating a graphical representation of the data. If the studied variable
...
variable. The greater the effect on the variance, the more the parameter explains the behavior of the variable.
...
to combine these to boost their performance. For this, many kinds of algorithms can be used
...
both of the prediction tasks. This is a good way of keeping the analysis reasonable as
the naive algorithms give a starting level for the required performance. If a hypothetically
...
Cross-validation can be a bit problematic if it is used both for choosing parameter values for algorithms (e.g., the value of $k$
for the kNN-algorithm) and evaluating the goodness of the different algorithms. In such a case the choice of the parameter value can make the estimator
of goodness biased, which is harmful when making objective decisions between the algorithms.

\subsection{Algorithm evaluation}

While using learning algorithms, type is not used as a predictor for quality and quality not for color.
If either quality or color was predicted very
accurately, there could be some benefits in using the first predictions in the second decision process.
However, there is a great amount of uncertainty involved in such a procedure.
If a test set differs significantly from the training set, this could make the second algorithm perform poorly.
...
problems. The greatest external utility is the set of the variables that is used for the predictions. 
...
most basic measure is the total amount of false predictions when the problem is treated as classification.
It does not give a complete picture of the situation. One can for instance predict that all the samples 
in the test data belong to one class. If
this class has a high prior probability, a large amount of predictions should be correct. On the other hand, a simple algorithm
could give a similar error rate as this naive assumption, but it would probably also have a non-zero success
...
the f-score should be relatively low.
...
would perform well in the given situation. Therefore, the algorithms have to be tested out.

The linear discriminant method is used to make a somewhat generic prediction for the classification. While the 
algorithms of matlab are used, also a quadratic discriminant
could be in principle equipped. This is not that advisable because the full quadratic form adds much unnecessary complexity
to the model.

To contribute to the results of a linear discriminant, a $k$ Nearest Neighbours -model is used. In this case it is important to choose
a fitting value for the parameter $k$. The 10-fold cross-validation method is fitting for this choice. It should be noted that if the model
...
Also the Random Forest algorithm is used. For this algorithm both the size
...
Primary component analysis can be useful in pre-processing data, in case \emph{unnecessary} data is given. Thus, it is good to check, whether
...
method may obtain significant advantage of having to use less dimensions. Thus, it is tested with different amounts of projection dimensions.
...
Various discriminative methods are used: linear discrimination, as well as the support vector machine and quadratic discrimination. 
...
While using regression algorithms there are two relevant questions: how does a regression algorithm perform in the context
...
In addition, the least squares method is utilized, as shown in \ref{method:leastsquare}. This is compulsory, taking
...
Based on the type prediction PCA results, it is determined whether or not it is useful to use PCA 
in this context. Moreover, algorithm combination has the same rules as in the case of 
...
The analyses of algorithmic performance were carried out so that the type was analyzed before the quality. The final learning algorithms
...
A cross-validation procedure was performed to find an optimal value for the parameter $k$ in the 
...
% Old dists: 0.0398, 0.0360, 0.0391, 0.0368, 0.0377, 0.0311, 0.0320, 0.0362
\begin{table}[H]
...
% Old dists: 0.0286, 0.0249, 0.0279, 0.0264, 0.0259, 0.0216, 0.0226, 0.02499
\begin{table}[H]
...
However, it should be noted that it is not nearly optimal model for the data in use. The non-Gaussian behavior
...
decided to combine all the three of them. At an error rate this low, it is
...
The challenges of determining the quality can be seen from Fig.
...
very clear separation present in this figure between different sections, in contrast to the quality of wines.
...
This time only the dummy model was supposedly worse than the others. There was no way to predict differences between the other models. The 
...
Only the linear discriminant of the methods involving discrimination obtained an f-score greater than 0.5 and even this with a small marginal.
...
In general the task of type prediction was completed well, with a small error rate. Similar results could have been
...
groups of wine tasters and the view of ranking varies to some extent. It is also possible that due to the complexity of the human
...
\caption{Color prediction results: dummy model}
\vspace{1.5mm}
...
\caption{Quality prediction results: dummy model}
